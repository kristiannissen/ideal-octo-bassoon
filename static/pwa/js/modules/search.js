...
class Search extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.root.querySelector("#query").addEventListener("keyup", (e) => {
      ...
      this.setAttribute("keyword", e.target.value.trim());
    });
  }
  ...
  set keyword(val) {
    if (val) this.setAttribute("keyword", val);
    else this.removeAttribute("keyword");
  }
  ...
  showTypeAhead(list) {
    ...
    let _list = list.map((item) => {
      return `<div class="typeahead-item">${item.Name}</div>`;
    });
